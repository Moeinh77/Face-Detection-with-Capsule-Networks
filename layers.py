...
def squash(s, axis=-1, epsilon=1e-7, name=None):
    ...
    with tf.variable_scope(name, default_name="squash"):
        ...
        # numerical issues such as divion by zero or vanishing gradients if 
        # s = 0
        norm = tf.sqrt(squared_norm + epsilon, name="norm")

        # squash(s) = squash_factor * unit_vector
...
def norm(s, axis=-1, epsilon=1e-7, keepdims=False, name=None):
    """Implements a numerically stable vector norm."""
    ...
    with tf.name_scope(name, default_name="safe_norm"):
        ...
        return tf.sqrt(squared_norm + epsilon, name="norm")



# TODO make these proper layers classes
        ...
        with tf.name_scope("routing"):
            ...
            def routing_iteration(counter, logits_old, outputs_old):
                with tf.name_scope("routing_iteration"):
                    ...
                    outputs = squash(weighted_sums, axis=-1, name="outputs")
